...
export class CartService {
	...
	addProduto(produto: ProdutoDTO): Cart {
		...
		let position = this.getProdutoPosition(cart, produto.id);
		if (position == -1) {
			cart.itens.push(
				{
					quantidade: 1,
	...
	getProdutoPosition(cart: Cart, produto_id: string) {
		return cart.itens.findIndex(x => x.produto.id == produto_id);
	}
}